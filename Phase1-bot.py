...
BOT_TOKEN = os.environ.get(“BOT_TOKEN”, “8606517814:AAEXhLHwG3XMGdmTDlanxHcksm_9qfPcxvU”)
...
if r['verdict'] == 'A+':
    ...
    msg += "⚠️ CAUTION — Partial confluence. Half size or wait."
else:
...
*PHASE 1 STEPS:*
1️⃣ Structure (HH/HL vs LL/LH)
2️⃣ Liquidity Sweep
3️⃣ Order Block
4️⃣ Displacement
5️⃣ Retracement into OB
6️⃣ Indicators (EMA/VWAP/RSI)
7️⃣ Targets (Entry/SL/TP1/2/3)
...
await update.message.reply_text(“🔍 Scanning watchlist… this takes ~30 seconds”)
results = []
...
results.append(f”{‘🟢’ if r[‘verdict’]==‘A+’ else ‘✅’} *{t}* — {r[‘verdict’]} | Score:{r[‘score’]}/12 | {‘📈’ if r[‘direction’]==‘bull’ else ‘📉’}”)
if results:
msg = “📊 *WATCHLIST SIGNALS*\n\n” + “\n”.join(results) + “\n\nType ticker name for full analysis”
else:
msg = “📊 *WATCHLIST SCAN COMPLETE*\n\nNo A+ or TAKE setups right now.\nMarket may be closed or choppy.\nTry again during NY session.”
await update.message.reply_text(msg, parse_mode=‘Markdown’)
...
if not ticker or len(ticker) < 1 or len(ticker) > 6:
    await update.message.reply_text("❓ Type a ticker to analyze. Example: SPY or BTC 5M")
    return

account = context.user_data.get('account', DEFAULT_ACCOUNT)
await update.message.reply_text(f"⏳ Analyzing *{ticker}* on {interval.upper()}...", parse_mode='Markdown')

data, err = fetch_data(ticker, interval)
if data is None:
    await update.message.reply_text(f"❌ Error: {err}\n\nTry:\n• Different ticker\n• Different timeframe (1H works best)\n• Check market hours")
    return

result = run_phase1(data, ticker)
if not result:
    await update.message.reply_text("❌ Not enough data — try 1H timeframe")
    return
